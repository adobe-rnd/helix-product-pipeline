...
/**
 * Keys generated by the pipeline that jsonldExtensions must not overwrite.
 * Callers needing full control should use the product.jsonld override instead.
 */
const RESERVED_JSONLD_KEYS = new Set([
  '@context', '@type', 'sku', 'name', 'description', 'gtin', 'url', 'brand', 'image', 'offers', 'custom',
]);
...
export function convertToJsonLD(state, product) {
  ...
  if (jsonldExtensions && typeof jsonldExtensions === 'object') {
    const safeExtensions = Object.fromEntries(
      Object.entries(jsonldExtensions).filter(([key]) => !RESERVED_JSONLD_KEYS.has(key)),
    );
    Object.assign(jsonld, safeExtensions);
  }
