...
/**
 * Helper to test URL rewriting: parse HTML to HAST, rewrite URLs, convert back to HTML
 * @param {string} html - Input HTML
 * @returns {string} - Output HTML after URL rewriting
 */
function rewriteHtml(html) {
  const hast = fromHtml(html, { fragment: true });
  rewriteContentImageUrls(hast);
  return toHtml(hast);
}
  ...
  describe('rewriteContentImageUrls', () => {
    describe('basic img src rewriting', () => {
      ...
      it('rewrites relative paths with query parameters', () => {
        ...
        const result = rewriteHtml(html);
        // HAST serialization encodes & as &#x26;
        assert.ok(result.includes('./content/content-images/media_1e34827d47552143150804dd7663928c2b4b88bf.jpg'));
        assert.ok(result.includes('width=2000'));
      });
    ...
    describe('query parameters', () => {
      it('rewrites images with query parameters', () => {
        ...
        const result = rewriteHtml(html);
        // HAST serialization encodes & as &#x26;
        assert.ok(result.includes('./products/images/content-images/media_88ff8b88feeefffcffabfeff358fffba98ff59ec7.jpg'));
      ...
      it('rewrites images with single query parameter', () => {
        ...
        const result = rewriteHtml(html);
        assert.ok(result.includes('./gallery/content-images/media_99ff9c99ffffffdffbcfeff469fffcba9aff6afd8.avif'));
        assert.ok(result.includes('width=2000'));
      });
      ...
      it('rewrites images with complex query strings', () => {
        ...
        const result = rewriteHtml(html);
        // HAST serialization encodes & as &#x26;
        assert.ok(result.includes('./assets/photos/content-images/media_aaffadaaffffffeffcdfeff57afffdbaabaff7bfe9.jpg'));
        assert.ok(result.includes('width=2000'));
      });
    });
    ...
    describe('source tag srcset', () => {
      ...
      it('rewrites source srcset with single quotes', () => {
        ...
        const result = rewriteHtml(html);
        // HAST normalizes quotes to double quotes
        assert.ok(result.includes('./products/content-images/media_ccffcfccffffffffffffffeff79bffffdcbdcff9dfb.jpg'));
      });
